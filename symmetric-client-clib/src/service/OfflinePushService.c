...
void SymOfflinePushService_execute(SymOfflinePushService *this, SymNode *node, SymRemoteNodeStatus *status) {
    ...
    do {
        ...
        SymOfflinePushService_pushToNode(this, node, status);
...
SymRemoteNodeStatuses * SymOfflinePushService_pushData(SymOfflinePushService *this, unsigned int force) {
        ...
        if (nodes->size > 0) {
            ...
            if (identitySecurity) {
                ...
                while (iter->hasNext(iter)) {
                    ...
                    SymOfflinePushService_execute(this, node, status);
                }
                identitySecurity->destroy(identitySecurity);
                iter->destroy(iter);
            } else {
