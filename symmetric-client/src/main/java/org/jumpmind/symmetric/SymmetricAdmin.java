...
public class SymmetricAdmin extends AbstractCommandLauncher {
    ...
    private void importCert(CommandLine line, List<String> args) {
        ...
        try {
            if (getSymmetricEngine().getParameterService().is(ServerConstants.HTTPS2_ENABLE, false)) {
                connection = new Http2Connection(url);
            } else {
        ...
        if (connection instanceof Http2Connection) {
            ((Http2Connection) connection).setSslSocketFactoryAndTrustManager(sslContext.getSocketFactory(), trustManager);
        } else {
        ...
        try {
            ...
            if (acceptAll) {
                ...
                if (trustedCertificate instanceof X509Certificate) {
                    ...
                    subject = x509Certificate.getSubjectDN().getName().replace(",", ", ");
                    issuer = x509Certificate.getIssuerDN().getName().replace(",", ", ");
                ...
                if (isValid) {
                    String answer;
                    while (true) {
                        answer = System.console().readLine("Accept this certificate? (Y/N): ");
