    ...
    public String transform(IDatabasePlatform platform,
        ...
        if (StringUtils.isNotBlank(sql)) {
            ...
            if (rowCount == 1) {
                ...
                lookupValue = values.get(0);
                log.warn("LookupColumnTransform.multipleRows", column.getTargetColumnName(),
                        column.getTransformId());
            } else if (values.size() == 0) {
                log.warn("LookupColumnTransform.noRows", column.getTargetColumnName(),
                        column.getTransformId());
            }
        } else {
            log.warn("LookupColumnTransform.noSql", column.getTargetColumnName(),
                    column.getTransformId());
        }
        return lookupValue;
    }
