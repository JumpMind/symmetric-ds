...
public class IntegrationTest {
    ...
    public void testSyncToClientAutoGeneratedPrimaryKey() {
        final String NEW_VALUE = "unique new value one value";
        rootJdbcTemplate.update(insertTestTriggerTableSql, new Object[] { "value one", "value \" two" });
        clientEngine.pull();
        rootJdbcTemplate.update(updateTestTriggerTableSql, new Object[] { NEW_VALUE });
        clientEngine.pull();
        Assert.assertEquals(clientJdbcTemplate.queryForInt("select count(*) from test_triggers_table where string_one_value=?", new Object[] {NEW_VALUE}), 1, "The update on test_triggers_table did not work.");
    }
