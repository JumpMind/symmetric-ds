...
public class ModelComparator {
    ...
    public List<ModelChange> compare(Database sourceModel, Database targetModel) {
        ...
        for (int tableIdx = 0; tableIdx < targetModel.getTableCount(); tableIdx++) {
            ...
            Table sourceTable = sourceModel.findTable(targetTable.getTableName(), caseSensitive);

            if (sourceTable == null) {
                if (log.isDebugEnabled()) {
                    log.debug("Table " + targetTable.getTableName() + " needs to be added");
                }
        ...
        for (int tableIdx = 0; tableIdx < sourceModel.getTableCount(); tableIdx++) {
            ...
            Table targetTable = targetModel.findTable(sourceTable.getTableName(), caseSensitive);

            if ((targetTable == null) && (sourceTable.getTableName() != null)
                    && (sourceTable.getTableName().length() > 0)) {
                if (log.isDebugEnabled()) {
                    log.debug("Table " + sourceTable.getTableName() + " needs to be removed");
                }
    ...
    public List<ModelChange> compareTables(Database sourceModel, Table sourceTable,
            ...
            if (sourceColumn == null) {
                if (log.isDebugEnabled()) {
                    log.debug("Column " + targetColumn.getName()
                            + " needs to be created for table " + sourceTable.getTableName());
                }
        ...
        Column[] sourcePK = sourceTable.getPrimaryKeyColumnsArray();
        Column[] targetPK = targetTable.getPrimaryKeyColumnsArray();
        ...
        if ((sourcePK.length == 0) && (targetPK.length > 0)) {
            ...
            if (changePK) {
                if (log.isDebugEnabled()) {
                    log.debug("The primary key of table " + sourceTable.getTableName()
                            + " needs to be changed");
                }
    ...
    public List<TableChangeImplBase> compareColumns(Table sourceTable, Column sourceColumn,
        ...
        if (targetColumn.getTypeCode() != sourceColumn.getTypeCode()
            ...
            if (log.isDebugEnabled()) {
                log.debug("The " + sourceColumn.getName() + " column on the "
                        + sourceTable.getTableName() + " table changed type codes from "
        ...
        if (sizeMatters && !StringUtils.equals(sourceColumn.getSize(), targetSize)) {
            if (log.isDebugEnabled()) {
                log.debug("The " + sourceColumn.getName() + " column on the "
                        + sourceTable.getTableName() + " table changed size from ("
        ...
        // size or precision
            ...
            if (log.isDebugEnabled()) {
                log.debug("The " + sourceColumn.getName() + " column on the "
                        + sourceTable.getTableName() + " table changed scale from ("
        ...
        if (sourceColumn.isRequired() != targetColumn.isRequired()) {
            if (log.isDebugEnabled()) {
                log.debug("The " + sourceColumn.getName() + " column on the "
                        + sourceTable.getTableName() + " table changed required status from "
        ...
        if (sourceColumn.isAutoIncrement() != targetColumn.isAutoIncrement()) {
            if (log.isDebugEnabled()) {
                log.debug("The " + sourceColumn.getName() + " column on the "
                        + sourceTable.getTableName() + " table changed auto increment status from "
