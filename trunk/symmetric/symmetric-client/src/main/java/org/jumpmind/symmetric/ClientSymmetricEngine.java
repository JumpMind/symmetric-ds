...
public class ClientSymmetricEngine extends AbstractSymmetricEngine {
			...
			for (String property : tokens) {
				...
				if (keyValue != null && keyValue.length > 1) {
					LoggerFactory.getLogger(ClientSymmetricEngine.class).info(
							"Setting database connection property %s=%s",
	...
	protected ISymmetricDialect createSymmetricDialect() {
		return new JdbcSymmetricDialectFactory(parameterService, platform)
				.create();
	}

	@Override
	protected IDatabasePlatform createDatabasePlatform(
			TypedProperties properties) {
		createDataSource(properties);
		waitForAvailableDatabase();
		return JdbcDatabasePlatformFactory.createNewPlatformInstance(
				this.dataSource, createDatabasePlatformSettings(properties));
	}

	protected DatabasePlatformSettings createDatabasePlatformSettings(
			TypedProperties properties) {
		DatabasePlatformSettings settings = new DatabasePlatformSettings();
		settings.setFetchSize(properties.getInt(
				ParameterConstants.DB_FETCH_SIZE, 1000));
		settings.setQueryTimeout(properties.getInt(
				ParameterConstants.DB_QUERY_TIMEOUT_SECS, 300));
		settings.setBatchSize(properties.getInt(
				ParameterConstants.JDBC_EXECUTE_BATCH_SIZE, 100));
		return settings;
	}
	...
	protected IStagingManager createStagingManager() {
		...
		return new StagingManager(memoryThresholdInBytes, timeToLiveInMs,
				directory);
	}
		...
		return new ITypedPropertiesFactory() {
			...
			protected Resource[] buildLocations(File propertiesFile) {
				...
				if (propertiesFile != null && propertiesFile.exists()) {
					resources.add(new FileSystemResource(propertiesFile
							.getAbsolutePath()));
				}
