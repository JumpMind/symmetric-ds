...
public class SecurityService implements ISecurityService {
	...
	protected Cipher getCipher(int mode) throws Exception {
		...
		log.info("Using {} algorithm provided by {}.", cipher.getAlgorithm(),
				cipher.getProvider().getName());
		return cipher;
	}

	protected void initializeCipher(Cipher cipher, int mode) throws Exception {
		AlgorithmParameterSpec paramSpec = Cipher
				.getMaxAllowedParameterSpec(cipher.getAlgorithm());
	...
	protected SecretKey getDefaultSecretKey() throws Exception {
		...
		KeySpec keySpec = new PBEKeySpec(keyPassword.toCharArray(),
				SecurityConstants.SALT, SecurityConstants.ITERATION_COUNT);
		return SecretKeyFactory.getInstance(SecurityConstants.ALGORITHM)
