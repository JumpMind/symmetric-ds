 ...
 */
public class TriggerRouterService extends AbstractService implements
	...
	public TriggerRouterService(IParameterService parameterService,
		...
		setSqlMap(new TriggerRouterServiceSqlMap(
				symmetricDialect.getPlatform(), createSqlReplacementTokens()));
	...
	public List<Trigger> getTriggers() {
		return sqlTemplate.query(
				"select "
						+ getSql("selectTriggersColumnList",
								"selectTriggersSql"), new TriggerMapper());
	}

	public boolean isTriggerBeingUsed(String triggerId) {
		return sqlTemplate.queryForInt(
				getSql("countTriggerRoutersByTriggerIdSql"), triggerId) > 0;
	}

	public boolean doesTriggerExist(String triggerId) {
		return sqlTemplate.queryForInt(getSql("countTriggerByTriggerIdSql"),
				triggerId) > 0;
	}

	public boolean doesTriggerExistForTable(String tableName) {
		return sqlTemplate.queryForInt(getSql("countTriggerByTableNameSql"),
				tableName) > 0;
	}

	public void deleteTrigger(Trigger trigger) {
		sqlTemplate.update(getSql("deleteTriggerSql"),
				(Object) trigger.getTriggerId());
	}

	protected void deleteTriggerHistory(TriggerHistory history) {
		sqlTemplate.update(getSql("deleteTriggerHistorySql"),
				history.getTriggerHistoryId());
	}

	public void createTriggersOnChannelForTables(String channelId,
			String catalogName, String schemaName, List<String> tables,
			String lastUpdateBy) {
	...
	public void createTriggersOnChannelForTables(String channelId,
			Set<Table> tables, String lastUpdateBy) {
	...
	public void inactivateTriggerHistory(TriggerHistory history) {
		sqlTemplate.update(getSql("inactivateTriggerHistorySql"), new Object[] {
				history.getErrorMessage(), history.getTriggerHistoryId() });
	}

	public Map<Long, TriggerHistory> getHistoryRecords() {
		final Map<Long, TriggerHistory> retMap = new HashMap<Long, TriggerHistory>();
		sqlTemplate.query(getSql("allTriggerHistSql"),
				new TriggerHistoryMapper(retMap));
		return retMap;
	}

	protected boolean isTriggerNameInUse(String triggerId, String triggerName) {
		return sqlTemplate.queryForInt(getSql("selectTriggerNameInUseSql"),
				triggerName, triggerName, triggerName, triggerId) > 0;
	}

	public TriggerHistory findTriggerHistory(String sourceTableName) {
		final Map<Long, TriggerHistory> retMap = new HashMap<Long, TriggerHistory>();
		sqlTemplate
				.query(getSql("allTriggerHistSql",
						"triggerHistBySourceTableWhereSql"),
						new TriggerHistoryMapper(retMap),
						new Object[] { sourceTableName },
	...
	public TriggerHistory getTriggerHistory(int histId) {
		...
		if (history == null && histId >= 0) {
			history = (TriggerHistory) sqlTemplate.queryForObject(
					getSql("triggerHistSql"), new TriggerHistoryMapper(),
					histId);
	...
	public TriggerHistory getNewestTriggerHistoryForTrigger(String triggerId) {
		return (TriggerHistory) sqlTemplate.queryForObject(
				getSql("latestTriggerHistSql"), new TriggerHistoryMapper(),
				triggerId);
	}
	...
	public List<TriggerHistory> getActiveTriggerHistories() {
		return sqlTemplate.query(
				getSql("allTriggerHistSql", "activeTriggerHistSql"),
				new TriggerHistoryMapper());
	}

	private String getNewestVersionOfRootConfigChannelTableNames() {
		TreeSet<String> ordered = new TreeSet<String>(
				rootConfigChannelTableNames.keySet());
		return ordered.last();
	}

	private String getMajorVersion(String version) {
		String majorVersion = Integer
				.toString(Version.parseVersion(version)[0]);
	...
	protected List<Trigger> buildTriggersForSymmetricTables(String version,
		...
		List<String> tables = new ArrayList<String>(
				rootConfigChannelTableNames.get(majorVersion));
		...
		if (tablesToExclude != null) {
			for (String tableToExclude : tablesToExclude) {
				String tablename = TableConstants.getTableName(tablePrefix,
						tableToExclude);
	...
	protected Trigger buildTriggerForSymmetricTable(String tableName) {
		boolean syncChanges = !TableConstants.getTablesThatDoNotSync(
				tablePrefix).contains(tableName)
				&& parameterService
						.is(ParameterConstants.AUTO_SYNC_CONFIGURATION);
		Trigger trigger = new Trigger();
	...
	public List<TriggerRouter> buildTriggerRoutersForSymmetricTables(
			String version, NodeGroupLink nodeGroupLink,
			String... tablesToExclude) {
		...
		for (int j = 0; j < triggers.size(); j++) {
			...
			TriggerRouter triggerRouter = buildTriggerRoutersForSymmetricTables(
					version, trigger, nodeGroupLink);
	...
	protected TriggerRouter buildTriggerRoutersForSymmetricTables(
			String version, Trigger trigger, NodeGroupLink nodeGroupLink) {
	...
	private String getTriggerRouterSql(String sql) {
		return getSql("select ", "selectTriggersColumnList", ",",
				"selectRoutersColumnList", ",",
				"selectTriggerRoutersColumnList", "selectTriggerRoutersSql",
				sql);
	}

	public List<TriggerRouter> getTriggerRouters() {
		return sqlTemplate.query(getTriggerRouterSql(),
				new TriggerRouterMapper());
	}

	public Set<TriggerRouter> getTriggerRouterForTableForCurrentNode(
			String catalogName, String schemaName, String tableName,
			boolean refreshCache) {
		return getTriggerRouterForTableForCurrentNode(null, catalogName,
				schemaName, tableName, refreshCache);
	}

	public Set<TriggerRouter> getTriggerRouterForTableForCurrentNode(
			NodeGroupLink link, String catalogName, String schemaName,
		...
		for (List<TriggerRouter> list : triggerRouters) {
			for (TriggerRouter triggerRouter : list) {
				if (isMatch(link, triggerRouter)
						&& isMatch(catalogName, schemaName, tableName,
	...
	protected boolean isMatch(NodeGroupLink link, TriggerRouter router) {
		if (link != null && router != null && router.getRouter() != null) {
			return link.getSourceNodeGroupId().equals(
					router.getRouter().getNodeGroupLink()
	...
	protected boolean isMatch(String catalogName, String schemaName,
			String tableName, Trigger trigger) {
		if (!StringUtils.isBlank(tableName)
				&& !tableName.equals(trigger.getSourceTableName())) {
			return false;
	...
	protected List<TriggerRouter> getConfigurationTablesTriggerRoutersForCurrentNode(
		...
		List<NodeGroupLink> links = configurationService
				.getNodeGroupLinksFor(sourceNodeGroupId);
	...
	protected void mergeInConfigurationTablesTriggerRoutersForCurrentNode(
		...
		for (TriggerRouter trigger : virtualConfigTriggers) {
			if (trigger.getRouter().getNodeGroupLink().getSourceNodeGroupId()
					...
					&& !doesTriggerRouterExistInList(configuredInDatabase,
							trigger)) {
	...
	protected boolean doesTriggerRouterExistInList(
			List<TriggerRouter> triggerRouters, TriggerRouter triggerRouter) {
		...
		if (cache == null
			...
			synchronized (this) {
				...
				for (TriggerRouter triggerRouter : triggerRouters) {
					String triggerId = triggerRouter.getTrigger()
							.getTriggerId();
					List<TriggerRouter> list = triggerRoutersByTriggerId
							.get(triggerId);
					if (list == null) {
				...
				newTriggerRouterCacheByNodeGroupId.put(myNodeGroupId,
						new TriggerRoutersCache(triggerRoutersByTriggerId,
								routers));
	 ...
	 *      boolean)
	 */
	public Router getActiveRouterByIdForCurrentNode(String routerId,
			boolean refreshCache) {
	...
	public List<Router> getRoutersByGroupLink(NodeGroupLink link) {
		return sqlTemplate.query(
				getSql("select", "selectRoutersColumnList",
						"selectRouterByNodeGroupLinkWhereSql"),
				new RouterMapper(), link.getSourceNodeGroupId(), link
						.getTargetNodeGroupId());
	}
	...
	public Trigger getTriggerById(String triggerId, boolean refreshCache) {
		...
		if (cache == null
			...
			synchronized (this) {
				...
				triggers.addAll(buildTriggersForSymmetricTables(Version
						.version()));
	...
	public List<Router> getRouters() {
		return sqlTemplate
				.query(getSql("select ", "selectRoutersColumnList",
						"selectRoutersSql"), new RouterMapper());
	}

	public List<TriggerRouter> getAllTriggerRoutersForCurrentNode(
			String sourceNodeGroupId) {
	...
	public List<TriggerRouter> getAllTriggerRoutersForReloadForCurrentNode(
		...
		return (List<TriggerRouter>) sqlTemplate.query(
				getTriggerRouterSql("activeTriggersForReloadSql"),
				new TriggerRouterMapper(), sourceNodeGroupId,
	...
	public TriggerRouter findTriggerRouterById(String triggerId, String routerId) {
		List<TriggerRouter> configs = (List<TriggerRouter>) sqlTemplate.query(
				getTriggerRouterSql("selectTriggerRouterSql"),
	...
	public Map<String, List<TriggerRouter>> getTriggerRoutersByChannel(
			String nodeGroupId) {
		...
		sqlTemplate.query(getTriggerRouterSql("selectGroupTriggersSql"),
				new TriggerRouterMapper() {
					...
					public TriggerRouter mapRow(Row rs) {
						...
						List<TriggerRouter> list = retMap.get(tr.getTrigger()
								.getChannelId());
						if (list == null) {
							list = new ArrayList<TriggerRouter>();
	...
	public void insert(TriggerHistory newHistRecord) {
		sqlTemplate.update(
				...
				new Object[] { newHistRecord.getTriggerId(),
						newHistRecord.getSourceTableName(),
						newHistRecord.getTableHash(),
						newHistRecord.getCreateTime(),
						...
						newHistRecord.getSourceCatalogName(),
						newHistRecord.getTriggerRowHash(),
						newHistRecord.getErrorMessage() }, new int[] {
	...
	public void deleteTriggerRouter(TriggerRouter triggerRouter) {
		sqlTemplate.update(getSql("deleteTriggerRouterSql"),
				(Object) triggerRouter.getTrigger().getTriggerId(),
	...
	public void saveTriggerRouter(TriggerRouter triggerRouter,
		...
		if (0 == sqlTemplate.update(
				...
				new Object[] { triggerRouter.getInitialLoadOrder(),
						...
						triggerRouter.getRouter().getRouterId() }, new int[] {
						Types.NUMERIC, Types.VARCHAR, Types.SMALLINT,
			...
			sqlTemplate.update(
					...
					new Object[] { triggerRouter.getInitialLoadOrder(),
							triggerRouter.getInitialLoadSelect(),
							triggerRouter.isPingBackEnabled() ? 1 : 0,
							triggerRouter.getCreateTime(),
							triggerRouter.getLastUpdateBy(),
							triggerRouter.getLastUpdateTime(),
							triggerRouter.getTrigger().getTriggerId(),
							triggerRouter.getRouter().getRouterId() },
					new int[] { Types.NUMERIC, Types.VARCHAR, Types.SMALLINT,
	...
	public void saveRouter(Router router) {
		...
		if (0 == sqlTemplate.update(
				...
				new Object[] { router.getTargetCatalogName(),
						...
						router.isSyncOnUpdate() ? 1 : 0,
						router.isSyncOnInsert() ? 1 : 0,
						router.isSyncOnDelete() ? 1 : 0,
						router.getLastUpdateBy(), router.getLastUpdateTime(),
						router.getRouterId() }, new int[] { Types.VARCHAR,
						Types.VARCHAR, Types.VARCHAR, Types.VARCHAR,
						Types.VARCHAR, Types.VARCHAR, Types.VARCHAR,
			...
			sqlTemplate.update(
					...
					new Object[] { router.getTargetCatalogName(),
							...
							router.getRouterExpression(),
							router.isSyncOnUpdate() ? 1 : 0,
							router.isSyncOnInsert() ? 1 : 0,
							router.isSyncOnDelete() ? 1 : 0,
							router.getCreateTime(), router.getLastUpdateBy(),
							router.getLastUpdateTime(), router.getRouterId() },
					new int[] { Types.VARCHAR, Types.VARCHAR, Types.VARCHAR,
							Types.VARCHAR, Types.VARCHAR, Types.VARCHAR,
							Types.VARCHAR, Types.SMALLINT, Types.SMALLINT,
							Types.SMALLINT, Types.TIMESTAMP, Types.VARCHAR,
	...
	public boolean isRouterBeingUsed(String routerId) {
		return sqlTemplate.queryForInt(
				getSql("countTriggerRoutersByRouterIdSql"), routerId) > 0;
	}

	public void deleteRouter(Router router) {
		if (router != null) {
			sqlTemplate.update(getSql("deleteRouterSql"),
					(Object) router.getRouterId());
		}
	}
	...
	public void saveTrigger(Trigger trigger) {
		...
		if (0 == sqlTemplate.update(
				...
				new Object[] { trigger.getSourceCatalogName(),
						trigger.getSourceSchemaName(),
						trigger.getSourceTableName(), trigger.getChannelId(),
						trigger.isSyncOnUpdate() ? 1 : 0,
						trigger.isSyncOnInsert() ? 1 : 0,
						trigger.isSyncOnDelete() ? 1 : 0,
						trigger.isSyncOnIncomingBatch() ? 1 : 0,
						trigger.isUseStreamLobs() ? 1 : 0,
						trigger.isUseCaptureLobs() ? 1 : 0,
						trigger.getNameForUpdateTrigger(),
						trigger.getNameForInsertTrigger(),
						trigger.getNameForDeleteTrigger(),
						trigger.getSyncOnUpdateCondition(),
						trigger.getSyncOnInsertCondition(),
						trigger.getSyncOnDeleteCondition(),
						trigger.getTxIdExpression(),
						trigger.getExcludedColumnNames(),
						trigger.getLastUpdateBy(), trigger.getLastUpdateTime(),
						trigger.getExternalSelect(), trigger.getTriggerId() },
				new int[] { Types.VARCHAR, Types.VARCHAR, Types.VARCHAR,
						Types.VARCHAR, Types.SMALLINT, Types.SMALLINT,
						Types.SMALLINT, Types.SMALLINT, Types.SMALLINT,
			...
			sqlTemplate
					.update(getSql("insertTriggerSql"),
							new Object[] { trigger.getSourceCatalogName(),
									trigger.getSourceSchemaName(),
									...
									trigger.getTxIdExpression(),
									trigger.getExcludedColumnNames(),
									trigger.getCreateTime(),
									trigger.getLastUpdateBy(),
									trigger.getLastUpdateTime(),
									trigger.getExternalSelect(),
		...
		if (clusterService.lock(ClusterConstants.SYNCTRIGGERS)) {
			synchronized (this) {
				try {
					...
					updateOrCreateDatabaseTriggers(triggersForCurrentNode,
							sqlBuffer, genAlways);
					resetTriggerRouterCacheByNodeGroupId();
				} finally {
	...
	protected Set<String> getTriggerIdsFrom(
			List<Trigger> triggersThatShouldBeActive) {
		Set<String> triggerIds = new HashSet<String>(
				triggersThatShouldBeActive.size());
	...
	protected void inactivateTriggers(List<Trigger> triggersThatShouldBeActive,
		...
		for (TriggerHistory history : activeHistories) {
			if (!triggerIdsThatShouldBeActive.contains(history.getTriggerId())) {
				...
				symmetricDialect.removeTrigger(sqlBuffer,
				...
				boolean triggerExists = symmetricDialect.doesTriggerExist(
						...
						history.getSourceTableName(),
						history.getNameForInsertTrigger());
				triggerExists |= symmetricDialect.doesTriggerExist(
						history.getSourceCatalogName(),
						history.getSourceSchemaName(),
						history.getSourceTableName(),
						history.getNameForUpdateTrigger());
	...
	protected List<Trigger> getTriggersForCurrentNode() {
		return new TriggerSelector(toList(getTriggerRoutersForCurrentNode(true)
				.values())).select();
	}

	protected void updateOrCreateDatabaseTriggers(List<Trigger> triggers,
			StringBuilder sqlBuffer, boolean genAlways) {
		...
		for (Trigger trigger : triggers) {
			...
			try {
				...
				Channel channel = configurationService.getChannel(trigger
						.getChannelId());
				if (channel == null) {
					errorMessage = String
				...
				if (table != null) {
					...
					boolean supportsTriggers = symmetricDialect.getPlatform()
							.getPlatformInfo().isTriggersSupported();
					...
					newestHistory = rebuildTriggerIfNecessary(sqlBuffer,
							...
							trigger.isSyncOnInsert() && supportsTriggers, table);

					newestHistory = rebuildTriggerIfNecessary(sqlBuffer,
							forceRebuildOfTriggers, trigger,
							DataEventType.UPDATE, reason,
							latestHistoryBeforeRebuild, newestHistory,
							trigger.isSyncOnUpdate() && supportsTriggers, table);

					newestHistory = rebuildTriggerIfNecessary(sqlBuffer,
							forceRebuildOfTriggers, trigger,
							DataEventType.DELETE, reason,
							latestHistoryBeforeRebuild, newestHistory,
							trigger.isSyncOnDelete() && supportsTriggers, table);
					...
					if (newestHistory != null) {
						...
						if (parameterService
								.is(ParameterConstants.AUTO_SYNC_TRIGGERS)) {
							if (this.triggerCreationListeners != null) {
								for (ITriggerCreationListener l : this.triggerCreationListeners) {
				...
				if (newestHistory != null) {
					...
					symmetricDialect.removeTrigger(null,
							trigger.getSourceCatalogName(),
							trigger.getSourceSchemaName(),
	...
	protected TriggerHistory rebuildTriggerIfNecessary(StringBuilder sqlBuffer,
			...
			TriggerReBuildReason reason, TriggerHistory oldhist,
			TriggerHistory hist, boolean triggerIsActive, Table table) {
		...
		newTriggerHist.setNameForInsertTrigger(getTriggerName(
				DataEventType.INSERT, maxTriggerNameLength, trigger)
				.toUpperCase());
		newTriggerHist.setNameForUpdateTrigger(getTriggerName(
				DataEventType.UPDATE, maxTriggerNameLength, trigger)
				.toUpperCase());
		newTriggerHist.setNameForDeleteTrigger(getTriggerName(
				DataEventType.DELETE, maxTriggerNameLength, trigger)
				.toUpperCase());
		...
		if (oldhist != null) {
			...
			triggerExists = symmetricDialect.doesTriggerExist(oldCatalogName,
			...
			triggerExists = symmetricDialect.doesTriggerExist(oldCatalogName,
					oldSourceSchema, trigger.getSourceTableName(),
					oldTriggerName);
		}
		...
		if ((forceRebuild || !triggerIsActive) && triggerExists) {
			symmetricDialect.removeTrigger(sqlBuffer, oldCatalogName,
					oldSourceSchema, oldTriggerName,
					trigger.getSourceTableName(), oldhist);
			triggerExists = false;
			triggerRemoved = true;
		}

		boolean isDeadTrigger = !trigger.isSyncOnInsert()
				&& !trigger.isSyncOnUpdate() && !trigger.isSyncOnDelete();
			...
			if (!triggerExists && triggerIsActive) {
				symmetricDialect
						.createTrigger(sqlBuffer, dmlType, trigger, hist,
								configurationService.getChannel(trigger
										.getChannelId()), tablePrefix, table);
				if (triggerRemoved) {
	...
	protected static String replaceCharsForTriggerName(String triggerName) {
		return triggerName
				.replaceAll("[^a-zA-Z0-9_]|[a|e|i|o|u|A|E|I|O|U]", "");
	}

	protected String getTriggerName(DataEventType dml,
			int maxTriggerNameLength, Trigger trigger) {
		...
		if (StringUtils.isBlank(triggerName)) {
			...
			String triggerSuffix1 = "on_" + dml.getCode().toLowerCase()
					+ "_for_";
			String triggerSuffix2 = replaceCharsForTriggerName(trigger
					.getTriggerId());
			...
			if (triggerName.length() > maxTriggerNameLength
					&& maxTriggerNameLength > 0) {
				triggerName = triggerPrefix1 + triggerSuffix1 + triggerSuffix2;
			}
		}
		...
		if (triggerName.length() > maxTriggerNameLength
			...
			log.debug(
					...
					new Object[] { dml.name().toLowerCase(),
							trigger.getTriggerId(), maxTriggerNameLength });
		}
	...
	class TriggerHistoryMapper implements ISqlRowMapper<TriggerHistory> {
		...
		public TriggerHistory mapRow(Row rs) {
			...
			hist.setNameForDeleteTrigger(rs
					.getString("name_for_delete_trigger"));
			hist.setNameForInsertTrigger(rs
					.getString("name_for_insert_trigger"));
			hist.setNameForUpdateTrigger(rs
	...
	class RouterMapper implements ISqlRowMapper<Router> {
		public Router mapRow(Row rs) {
			...
			router.setNodeGroupLink(configurationService.getNodeGroupLinkFor(
					rs.getString("source_node_group_id"),
					rs.getString("target_node_group_id")));
	...
	class TriggerMapper implements ISqlRowMapper<Trigger> {
		public Trigger mapRow(Row rs) {
			...
			trigger.setSyncOnIncomingBatch(rs
					.getBoolean("sync_on_incoming_batch"));
			trigger.setUseStreamLobs(rs.getBoolean("use_stream_lobs"));
			trigger.setUseCaptureLobs(rs.getBoolean("use_capture_lobs"));
			trigger.setNameForDeleteTrigger(rs
					.getString("name_for_delete_trigger"));
			trigger.setNameForInsertTrigger(rs
					.getString("name_for_insert_trigger"));
			trigger.setNameForUpdateTrigger(rs
					.getString("name_for_update_trigger"));
			...
			trigger.setExcludedColumnNames(rs
					.getString("excluded_column_names"));
	...
	class TriggerRouterMapper implements ISqlRowMapper<TriggerRouter> {
		...
		public TriggerRouter mapRow(Row rs) {
			...
			triggerRouter.setInitialLoadSelect(rs
					.getString("initial_load_select"));
			triggerRouter
					.setPingBackEnabled(rs.getBoolean("ping_back_enabled"));
	...
	public void setRootConfigChannelTableNames(
			Map<String, List<String>> configChannelTableNames) {
