...
public class IntegrationTest extends AbstractIntegrationTest implements ITest {
    ...
    public String getTestName() {
        try {
            return "Test from " + getRootDatabaseName() + " to "
                    + getClientDatabaseName();
    ...
    protected void init() {
        ...
        BeanFactory clientBeanFactory = getClientEngine()
    ...
    protected void register() {
        ...
        Assert.assertTrue(getClientEngine().isRegistered(),
        ...
        if (getRootDbDialect().isClobSyncSupported()) {
            Assert
                    .assertEquals(
                            clientJdbcTemplate
                                    .queryForObject(
                                            "select notes from test_customer where customer_id=101",
    ...
    protected void testSyncToRootAutoGeneratedPrimaryKey() {
        ...
        clientJdbcTemplate.update(insertTestTriggerTableSql, new Object[] {
                "value one", "value \" two" });
        getClientEngine().push();
        clientJdbcTemplate.update(updateTestTriggerTableSql,
    ...
    protected void testSyncInsertCondition() throws ParseException {
        ...
        rootJdbcTemplate.update(insertOrderHeaderSql, new Object[] { "12", 100,
                "C", date }, new int[] { Types.VARCHAR, Types.INTEGER,
                Types.CHAR, Types.DATE });
        getClientEngine().pull();
        Assert.assertEquals(clientJdbcTemplate.queryForList(
    ...
    protected void testIgnoreNodeChannel() {
        INodeService nodeService = (INodeService) getRootEngine()
                .getApplicationContext().getBean("nodeService");
        nodeService.ignoreNodeChannelForExternalId(true,
        ...
        nodeService.ignoreNodeChannelForExternalId(false,
                TestConstants.TEST_CHANNEL_ID,
                TestConstants.TEST_ROOT_NODE_GROUP,
                TestConstants.TEST_ROOT_EXTERNAL_ID);
    ...
    protected void testHeartbeat() throws Exception {
        ...
        getClientEngine().push();
        Date time = (Date) rootJdbcTemplate.queryForObject(
                "select heartbeat_time from " + TestConstants.TEST_PREFIX
                        + "node where external_id='"
                        + TestConstants.TEST_CLIENT_EXTERNAL_ID + "'",
                Timestamp.class);
