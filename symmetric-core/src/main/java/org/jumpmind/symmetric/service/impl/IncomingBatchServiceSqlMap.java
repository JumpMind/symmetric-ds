...
"select create_time from $(schemaName)$(incoming_batch)   " );
...
"select count(*) from $(schemaName)$(incoming_batch) where error_flag=1   " );

        putSql("countIncomingBatchesErrorsOnChannelSql" ,"" + 
"select count(*) from $(schemaName)$(incoming_batch) where error_flag=1 and channel_id=?");
        
        putSql("insertIncomingBatchSql" ,"" + 
"insert into $(schemaName)$(incoming_batch) (batch_id, node_id, channel_id, status, network_millis, filter_millis, load_millis, failed_row_number, failed_line_number, byte_count, " + 
...
"update $(schemaName)$(incoming_batch) set status = ?, error_flag=?, network_millis = ?, filter_millis = ?, load_millis = ?, failed_row_number = ?, failed_line_number = ?, byte_count = ?, " + 
...
"delete from $(schemaName)$(incoming_batch) where batch_id = ? and node_id = ? " );

        putSql("deleteIncomingBatchByNodeSql" ,"delete from $(schemaName)$(incoming_batch) where node_id = ?");
        
        putSql("maxBatchIdsSql", "select max(batch_id) as batch_id, node_id, channel_id from $(schemaName)$(incoming_batch) where status = ? group by node_id, channel_id");
        ...
        putSql("selectIncomingBatchSummaryByStatusAndChannelSql",
                "select count(*) as batches, s.status, sum(s.load_row_count) as data, s.node_id, min(s.create_time) as oldest_batch_time, s.channel_id,      "
                        ...
                        + "  from $(schemaName)$(incoming_batch) s join $(schemaName)$(incoming_batch) b on b.batch_id=s.batch_id and b.node_id=s.node_id "
                        + "  join $(schemaName)$(node) n on n.node_id=b.node_id and n.sync_enabled=1 where s.status in (:STATUS_LIST) group by s.status, s.node_id, s.channel_id, b.sql_message order by oldest_batch_time asc   ");
        ...
        putSql("selectIncomingBatchSummaryByStatusSql",
                "select count(*) as batches, status, sum(load_row_count) as data, node_id, min(create_time) as oldest_batch_time,      "
                        ...
                        + "  from $(schemaName)$(incoming_batch) where status in (:STATUS_LIST) group by status, node_id order by oldest_batch_time asc   ");

        putSql("lastUpdateByChannelSql", "select max(last_update_time) as last_update_time, channel_id from $(schemaName)$(incoming_batch) group by channel_id");
        
        putSql("getAllBatchesSql", "select batch_id, node_id from $(schemaName)$(incoming_batch)");
