...
public class FileSyncServiceSqlMap extends AbstractSqlMap {
    ...
    public FileSyncServiceSqlMap(IDatabasePlatform platform, Map<String, String> replacementTokens) {
        ...
        putSql("selectFileTriggersSql",
        		...
        		" from $(schemaName)$(file_trigger)                                                        ");

        putSql("triggerIdWhere", "where trigger_id=?");

        putSql("updateFileTriggerSql",
                " update $(schemaName)$(file_trigger) set base_dir=?, recurse=?, includes_files=?,         " +
        ...
        putSql("insertFileTriggerSql",
                " insert into $(schemaName)$(file_trigger) (base_dir, recurse, includes_files,             " +
                "  excludes_files, sync_on_create, sync_on_modified, sync_on_delete,          " +
                "  sync_on_ctl_file, delete_after_sync,                                       " +
                "  before_copy_script, after_copy_script,                                     " +
        ...
        putSql("selectFileSnapshotSql",
                ...
                " from $(schemaName)$(file_snapshot) where trigger_id=? and router_id=?                             ");
        
        putSql("relativeDirWhere", "and relative_dir=?");

        putSql("updateFileSnapshotSql",
                " update $(schemaName)$(file_snapshot) set   " +
                        "  last_event_type=?, crc32_checksum=?,                                 " +
        ...
        putSql("updateFileIncoming",
                " update $(schemaName)$(file_incoming) set                                                   " +
                        "  node_id=?,                                                           " +
                        "  file_modified_time=?,                                                " +
                        "  last_event_type=?                                                    " +
                        " where                                                                 " +
        ...
        putSql("insertFileIncoming",
                " insert into $(schemaName)$(file_incoming) (node_id, file_modified_time, last_event_type, relative_dir, file_name) " +
                "   values(?,?,?,?,?)                                                                             ");

        putSql("deleteFileIncoming",
                " delete from $(schemaName)$(file_incoming)");

        putSql("findNodeIdFromFileIncoming",
                " select node_id from $(schemaName)$(file_incoming) where relative_dir=? and file_name=? and file_modified_time=?");

        putSql("deleteFileSnapshotSql",
                " delete from $(schemaName)$(file_snapshot)                                                  " +
                        " where                                                                 " +
                        "  trigger_id=? and router_id=? and relative_dir=? and file_name=?         ");

        putSql("insertFileSnapshotSql",
                " insert into $(schemaName)$(file_snapshot) (                                                " +
                "  last_event_type, crc32_checksum,                                             " +
        ...
        putSql("insertFileTriggerRouterSql",
                " insert into $(schemaName)$(file_trigger_router) (                                          " +
                        "  enabled, initial_load_enabled, target_base_dir,                      " +
        ...
        putSql("deleteFileTriggerRouterSql", ""
                + "delete from $(schemaName)$(file_trigger_router) where trigger_id=? and router_id=? ");

        putSql("deleteAllFileTriggerRoutersSql", "delete from $(schemaName)$(file_trigger_router)");

        putSql("deleteFileTriggerSql", "" + "delete from $(schemaName)$(file_trigger) where trigger_id=?   ");
    }
