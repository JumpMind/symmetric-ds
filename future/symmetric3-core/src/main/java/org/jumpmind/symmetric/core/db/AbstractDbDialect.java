...
abstract public class AbstractDbDialect implements IDbDialect {
	...
	public AbstractDbDialect(Parameters parameters) {
		...
		this.databaseDefinition = new SymmetricTables(parameters.get(
				Parameters.DB_TABLE_PREFIX, SymmetricTables.DEFAULT_PREFIX));
	}
		...
		for (int i = 0; i < values.length; i++) {
			...
			try {
				if (column != null) {
					...
					if ((value == null || (dialectInfo.isEmptyStringNulled() && value
							.equals("")))
							&& column.isRequired()
							&& column.isOfTextType()) {
					...
					if (value != null) {
						if (type == Types.DATE
								&& !dialectInfo.isDateOverridesToTimestamp()) {
							objectValue = getDate(value, TIMESTAMP_PATTERNS);
						} else if (type == Types.TIMESTAMP
								|| (type == Types.DATE && dialectInfo
							...
							if (encoding == BinaryEncoding.NONE) {
								...
								objectValue = Base64.decodeBase64(value
										.getBytes());
							...
							objectValue = new Time(
									getTime(value, TIME_PATTERNS));
				...
				log.log(LogLevel.ERROR,
						"Could not convert a value of %s for column %s of type %s",
	...
	public String getConstraintName(String prefix, Table table,
		...
		return shortenName(result.toString(), getDbDialectInfo()
	...
	public Table findTable(String catalogName, String schemaName,
			String tableName, boolean useCached) {
		Table cachedTable = cachedModel.findTable(catalogName, schemaName,
				tableName);
		if (cachedTable == null || !useCached) {
			Table justReadTable = readTable(catalogName, schemaName, tableName,
					...
					parameters.is(
							Parameters.DB_USE_ALL_COLUMNS_AS_PK_IF_NONE_FOUND,
							false));
	...
	abstract protected Table readTable(String catalogName, String schemaName,
			String tableName, boolean caseSensitive,
			boolean makeAllColumnsPKsIfNoneFound);
	...
	public DmlStatement createDmlStatement(DmlType dmlType, Table table,
		...
		return createDmlStatement(dmlType, table.getFullyQualifiedTableName(),
				table.getColumnSet(columnFilter, true),
				table.getColumnSet(columnFilter, false));
	}

	public DmlStatement createDmlStatement(DmlType dmlType, String tableName,
			Column[] keys, Column[] columns) {
		return createDmlStatement(dmlType, tableName, keys, columns, null);
	}

	public DmlStatement createDmlStatement(DmlType dmlType, String tableName,
			Column[] keys, Column[] columns, Column[] preFilteredColumns) {
